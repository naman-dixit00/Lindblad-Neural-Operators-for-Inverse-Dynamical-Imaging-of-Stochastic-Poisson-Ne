...
Includes L1 structural sparsity regularization for dynamic gating pruning.
"""
...
class TotalLNOLoss(nn.Module):
    def __init__(self, model, lambda_recon: float = 1.0, lambda_diss: float = 0.2,
                 lambda_entropy: float = 0.05, lambda_sparsity: float = 0.05,
                 dx: float = 1.0, dt: float = 1.0, **kwargs):
        """
        Combines structural reconstruction data loss with non-equilibrium thermodynamic
        constraints and structural L1 sparsity gating regularization.
        """
        super(TotalLNOLoss, self).__init__()
        self.model = model # Reference to LNO architecture
        self.lambda_recon = lambda_recon
        self.lambda_diss = lambda_diss
        self.lambda_entropy = lambda_entropy
        self.lambda_sparsity = lambda_sparsity # Structural pruning coefficient
    ...
    def forward(self, *args, **kwargs):
        ...
        physics_loss = self.pde_loss(inputs, targets_pred, targets_true)

        # 8. Structural Sparsity Penalty (L1 norm of gated gamma vectors)
        sparsity_loss = self.model.get_structural_sparsity_penalty()

        # 9. Total Multi-Objective Formulation
        total_loss = (self.lambda_recon * recon_loss) + (self.lambda_diss * physics_loss) + (self.lambda_sparsity * sparsity_loss)
